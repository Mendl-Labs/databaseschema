//! Deployment position model. No tenant_id. Maintained by the average-cost
//! engine in [`crate::ops::deployment_position_ops`].
...
#[diesel(table_name = crate::schema::deployment_positions)]
...
pub struct DeploymentPosition {
    ...
    /// Links two sibling-leg rows of one pairs-trading position. `None` for
    /// every ordinary (non-pair) position.
    pub pair_group_id: Option<Uuid>,
    /// "pair_long" or "pair_short". `None` when `pair_group_id` is `None`.
